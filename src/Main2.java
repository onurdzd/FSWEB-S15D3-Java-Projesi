...
public class Main2 {
    public static void main(String[] args) {
        ...
        repeatedWordCount(text);
    }
    public static void repeatedWordCount(String text){
       String[] splittedText=text.split(" ");
        Map<String,Integer>repeatedWords=new HashMap<>();
      for(int i=0;i<splittedText.length;i++){
          splittedText[i]=splittedText[i].toLowerCase();
          splittedText[i]= splittedText[i].replaceAll("[^a-zA-Z]", "");
          if(!splittedText[i].isEmpty()){
              repeatedWords.put(splittedText[i],repeatedWords.getOrDefault(splittedText[i],0)+1); // her tekrar eden itemi görünce +1 ekliyor
          }
      }
